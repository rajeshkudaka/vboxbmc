...
from vboxbmc.vbmc import VBoxBMC
...
class VBpxBMCManager(object):
    ...
    def __init__(self):
        super(VBoxBMCManager, self).__init__()
    ...
    def _store_config(self, **options):
        ...
        config_path = os.path.join(
            self.config_dir, options['vm_name'], 'config'
        )
    ...
    def _sync_vbmc_states(self, shutdown=False):
        ...
        def vbmc_runner(bmc_config):
            ...
            try:
                ...
                LOG.exception(
                    'Shutdown vBMC for vm %(vm)s, cause '
                    '%(error)s', {'vm': show_options['vm'],
                                  'error': ex}
                )
                return
        ...
        for vm_name in os.listdir(self.config_dir):
            if not os.path.isdir(
                    os.path.join(self.config_dir, vm_name)
            ):
                continue
            ...
            if lets_enable:
                ...
                if not instance or not instance.is_alive():
                    ...
                    LOG.info(
                        'Started vBMC instance for vm '
                        '%(vm)s', {'vm': vm_name}
                    )

                if not instance.is_alive():
                    LOG.debug(
                        'Found dead vBMC instance for vm %(vm)s '
                        '(rc %(rc)s)', {'vm': vm_name,
                ...
                if instance:
                    if instance.is_alive():
                        instance.terminate()
                        LOG.info(
                            'Terminated vBMC instance for vm '
                            '%(vm)s', {'vm': vm_name}
                        )
	...
	# ADD: check if the vm exists
        ...
        try:
            ...
            msg = ('Failed to create vm %(vm)s. '
                   'Error: %(error)s' % {'vm': vm_name, 'error': ex})
            LOG.error(msg)
            return 1, msg

        try:
            self._store_config(vm_name=vm_name,
                               username=username,
                               password=password,
                               port=str(port),
                               address=address,
    ...
    def delete(self, vm_name):
        ...
        if not os.path.exists(vm_path):
            raise exception.DomainNotFound(vm=vm_name)

        try:
            self.stop(vm_name)
        except exception.VBoxBMCError:
            pass
    ...
    def start(self, vm_name):
        ...
        try:
            self._vbmc_enabled(vm_name,
                               config=bmc_config,
                               lets_enable=True)

        except Exception as e:
            LOG.exception('Failed to start vm %s', vm_name)
            return 1, ('Failed to start vm %(vm)s. Error: '
