...
from vboxbmc.manager import VBoxBMCManager
...
def main_loop(vbmc_manager, handle_command):
    ...
    try:
        ...
        while True:
            ...
            except exception.VBoxBMCError as ex:
...
def command_dispatcher(vbmc_manager, data_in):
    ...
    if command == 'add':
        ...
        data_out = [vbmc_manager.delete(vm_name)
                    for vm_name in set(data_in['vm_names'])]
        return {
        ...
        data_out = [vbmc_manager.start(vm_name)
                    for vm_name in set(data_in['vm_names'])]
        return {
            'rc': max(rc for rc, msg in data_out),
            'msg': [msg for rc, msg in data_out if msg],
        ...
        data_out = [vbmc_manager.stop(vm_name)
                    for vm_name in set(data_in['vm_names'])]
        return {
            'rc': max(rc for rc, msg in data_out),
            'msg': [msg for rc, msg in data_out if msg],
